...
(X ≈ 0.50–0.52 at sites 31, 32, 35, 36) around a central dip (X ≈ −0.002 at 33, 34).
...
fully solved fork by a member of a winning Track-B team is public (see
`../CANDIDATE_ASSESSMENT.md`). The QDC grader endpoint (`qc_grader.challenges.qdc_2025`) was
...
* The QDC helper `postselection_and_mitigation` is *given* in `challenge_utils.py`, but ex 3.1 requires
  ...
  score in ex 4.2 is recomputed by the grader from the raw evs *with the team's own* `odr_mitigate`.
...
| | 3.2 | `twirl_circuit`, `postselect_charge`, charge witness | 5 | auto |
...
```
RMSE_W   = sqrt(mean_{j∈W} (X_hat_j − X_ref_j)^2)       (NaN excluded, −5 % per NaN in W)
C        = mean(X_hat[31,32,35,36]) − mean(X_hat[33,34]),  C_ref = 0.515 (SPEC; bond-64 value 0.512)
...
quiet 0.021 → 5.0 + 3.1 + 2 + 2 ≈ 12/18 with the formula as written (SPEC quotes ≈ 10/18 assuming the
quiet term is lost; see §12). Raw twirl+DD without ODR: RMSE_W 0.229, C 0.068 → ≈ 1/18. A team at
...
| Day 1 17:00–19:00 | Part 3.1–3.2 (ODR + twirling) | level-2 hints for 2.2 and 3.1 released at 18:00 (§7) |
...
| **Day 2 15:00** | **Hardware freeze** — no further jobs count | anything submitted after 15:00 is ignored by the grader (`job_info.submitted`) |
...
`job.usage()` in `job_info["usage_s"]`; (3) `grade_submission.py` re-fetches `usage()` for every job
id and subtracts 5 points from ex 4.2 if the team total exceeds 180 s.
...
1. `organizer/fallback_data/` holds the raw July-2026 ibm_kingston set and the ibm_boston canary.
   Run `python organizer/make_hardware_fallback.py` (writes `reference_data/hardware_fallback.npz`
   with the ODR T = 8 evs/stds, layout, job id, `fallback=True`).
2. Hand the file **only** to teams without a returned main job at Day 2 09:00 (copy into their
   `reference_data/`); the notebook's guarded hardware cell picks it up automatically when
   `RUN_ON_HARDWARE = False`.
3. Their ex 4.2 score is capped at 75 % (13.5/18) and the leaderboard line is flagged `fallback`.
   They still must run *their own* `odr_mitigate` on it (the grader recomputes X_mit).
...
submission/            score.json, ex*.npz written by fallfest_grader (do not edit)
                       hardware_result.npz, canary_result.npz, improvement.npz, job_info.json
schwinger_hadron_participant.ipynb     executed, outputs included
report.pdf             ≤ 2 pages: error-budget table, ODR bias derivation, improvement rationale
```

**Run the organizer grader** (conda env `qiskit-paper`, offline unless `--live`):

```
python organizer/grade_submission.py path/to/<team>.zip                 # JSON + leaderboard line
python organizer/grade_submission.py path/to/<team>/ --hidden           # hidden mode (below)
python organizer/grade_submission.py submissions/*.zip --leaderboard leaderboard.csv
python organizer/grade_submission.py <team>.zip --live                  # re-fetch job usage/metadata
```

* It re-runs every `grade_exN` on the saved inputs (`submission/exN.npz`) and re-imports the team's
  functions from the notebook (`odr_mitigate`, `select_chain`, `evolve_circuits_matched`, …) with
  `nbconvert --to script` in a subprocess with a 5-minute timeout, no network.
* **Hidden mode** (`--hidden`) swaps the fixed seeds and lattice sizes used by the public grader for a
  second set (L = 10 for the state-prep fingerprints, a different random angle, different synthetic
  noise for 3.1, a different frozen calibration snapshot for 2.3) so that anything hard-coded against
  the public `grader_refs.npz` is caught. Report hidden-mode points as the official score.
* **Leaderboard**: rank by ex 4.2 `RMSE_W`, ties by RMSE over all 68 sites, then by lower total usage.
  `fallback` rows are listed but ranked below every genuine row.
* **Integrity checks** (each failure → 0 points on the affected exercise and a judge flag):
  1. `X_hat` recomputed from the submitted 4 PUB evs with the team's `odr_mitigate` must equal the
     submitted `X_mit` to 1e-6.
  2. All four PUB circuits share one `initial_layout` (68 distinct physical qubits, a connected path
     on the backend's coupling map) and the observables are `apply_layout`-consistent with it.
  3. `usage_s` per job matches `job.usage()` within 10 % when re-fetched; team total ≤ 180 s.
  4. The submitted evs are not identical (to 1e-9, any of the 68 sites) to any array in the shipped
     `hardware_ibm_kingston_2026-07-25.npz` or the fallback file unless `fallback=True` is declared.
     (The shipped npz contains *all* strategies — baseline, twirl_dd, odr, trex, zne_fold, … —
     so this check matters.)
  5. Job timestamps lie inside the hardware windows; a job submitted after the freeze is ignored.
...
post-selection/mirror averaging. Expected magnitudes in §11.
...
11. ★★ *Write the ODR formula and say what the post-selection on f does.* — With chi = (−1)^j Z + 1,
...
18. ★ *What is the charge witness and what does `postselect_charge` do?* — Total charge
    Q = −½ Σ_k [Z_k + (−1)^k] is conserved (Q = 0 sector); in a Sampler run, bit-strings with Q ≠ 0
    are noise and are discarded; the fraction discarded is a witness of the error rate (expected ≈
    1 − retention). With twirling the witness ordering (twirled ≥ untwirled discard) is the B2 point.
...
4. **Local testing mode ignores every option except `shots`.** Running `EstimatorV2(mode=FakeKingston())`
   ...
   must use `twirl_circuit` (ex 3.2) explicitly and an Aer `NoiseModel`. Do not let a team "verify"
   twirling on a fake backend.
...
All commands from the project root in the `qiskit-paper` env (Python 3.13, Qiskit 2.5.2, runtime
0.49.0, Aer 0.17.2). Timings are on an 8-core MacBook.

| command | output | time |
|---|---|---|
| `python tools/test_reference.py` | verifies `schwinger_reference.py` (must print `ALL TESTS PASSED`) | ≈ 2 min (t = 8 bond-40 pair ≈ 30 s) |
| `python organizer/make_grader_refs.py` | `reference_data/grader_refs.npz` (fingerprints, exact states, Trotter table, frozen FakeKingston target summary) | ≈ 2–4 min (L = 8 exact evolution is the slow part) |
| `python tools/make_mps_reference.py` | `reference_data/mps_reference_L34.npz` + `_meta.json` (bond 64 and 128, t = 2, 4, 6, 8) | ≈ 50 min total with the even-first step (bond 64 → 55 / 170 / 277 / 369 s; bond 128 → 90 / 475 / 624 / 887 s); the pinned odd-first step ran bond 64 t = 8 in 125 s, so expect less |
| `python organizer/make_hardware_fallback.py` | `reference_data/hardware_fallback.npz` from `organizer/fallback_data/` | seconds |
| `python tools/build_notebooks.py` | both notebooks, unexecuted | seconds |
| `python tools/build_notebooks.py --execute` | executes the solution notebook in place (`nbconvert`, kernel `python3`, per-cell timeout 3600 s, `MPLBACKEND=Agg`) | < 25 min offline (SPEC target < 20) |
| `MPLBACKEND=Agg python scratch/trotter_ordering_check.py` | the two ordering deviations of §12 (the script labels its own `trotter_step_alt` "odd-first" and the module's step "pinned"; after the 2026-09-07 re-pin the module *is* odd-first, so read the labels accordingly) | ≈ 1 min |

`mps_reference_L34.npz` and `grader_refs.npz` (which copies `X_ref_t8_bd64`, `C_ref`, `RMSE_0` from
it) must be regenerated after any change to `trotter_step`; run `make_mps_reference` first, then
`make_grader_refs`.

Order for a fresh machine: `test_reference` → `make_grader_refs` → `make_mps_reference`
(background) → `build_notebooks --execute`. The participant kit is the project folder minus
`organizer/`, `tools/`, `scratch/` and the solution notebook.
...
| MPS timings (t = 8, two circuits, 8 cores) | bond 40 ≈ 30 s, bond 64 = 125 s (pinned; 369 s in the even-first run), bond 128 = 887 s (even-first run) | ex 1.5 |
...
1. **Trotter ordering — measured deviations (the "builder task" of SPEC §E).** With Aer MPS at bond
   40 (seed 7), L = 34, t = 8, 8 steps of dt = 1 (`scratch/trotter_ordering_check.py`):
   ...
   before the grader build. **Open item:** `reference_data/mps_reference_L34.npz` (written 14:44)
   and the `X_ref_t8_bd64` / `C_ref` / `RMSE_0` copies inside `grader_refs.npz` (14:51) were computed
   with the *even-first* step and must be regenerated (`make_mps_reference` 50 min → `make_grader_refs`
   3 min) before the notebooks are executed; the answer-key X_ref row in §11 gives both sets of values.
2. `X_ref` constants: with the pinned (odd-first) ordering the bond-64 reference has peaks
   0.509 / 0.507, dip −0.006, C_ref = 0.514, RMSE_0 = 0.269 — consistent with SPEC §E (0.51, −0.006,
   0.515, 0.269). The currently shipped (stale, even-first) `mps_reference_L34.npz` gives
   0.525 / 0.496, −0.002, 0.512, 0.270 instead; regenerate as in item 1. The grader should read the
   constants from `grader_refs.npz` rather than hard-code them.
...
4. SPEC §B says the solution notebook must execute in < 20 min; the build brief allows < 25 min
   (bond-64 MPS at t = 8 alone is 6 min, so the notebook loads the organizer arrays). Use 25.
