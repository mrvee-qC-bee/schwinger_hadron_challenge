...
def grade_ex0_3(prep_vacuum) -> float:
    ...
    for L in (6, 8):
        ...
        f = _fidelity(_statevector(qc), r[f"sv_vacuum_L{L}"]); fids[L] = f
        if f > 1 - 1e-9:
            pts += 1.0
        else:
...
def grade_ex0_4(prep_wave) -> float:
    ...
    for L in (6, 8):
        ...
        f = _fidelity(_statevector(qc), r[f"sv_wave_L{L}"]); fids[L] = f
        fv = _fidelity(_statevector(qc), r[f"sv_vacuum_L{L}"])
        if f > 1 - 1e-9:
            pts += 1.0
        elif fv > 1 - 1e-9:
...
def grade_ex0_5(trotter_step, evolve_circuits, prep_wave) -> float:
    ...
    init_ok = err is None and isinstance(qi, QuantumCircuit) and qi.num_qubits == n and _fidelity(_statevector(qi), r["sv_wave_L6"]) > 1 - 1e-9
    if not init_ok:
    ...
    if err or not (isinstance(res, (tuple, list)) and len(res) == 2 and all(isinstance(c, QuantumCircuit) for c in res)):
        ...
        qc_phys = res[0]
        psi = Statevector(psi0).evolve(qc_phys).data if not init_ok else _statevector(qc_phys)
        ...
        if err4 or not (isinstance(res4, (tuple, list)) and len(res4) == 2):
            ...
            qm = res4[1]
            psi_m = Statevector(psi0).evolve(qm).data if not init_ok else _statevector(qm)
