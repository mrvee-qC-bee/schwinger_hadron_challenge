...
for olevel in [0, 1]:
    pm=generate_preset_pass_manager(optimization_level=olevel, basis_gates=["rz","sx","x","cz"], seed_transpiler=1)
    circ=[pm.run(c) for c in (qwp,qwm,qvp,qvm)]
    print("O",olevel,"cz counts",[c.count_ops().get('cz') for c in circ])
    for p in [0.005, 0.01]:
        ...
        for nname,nm in [("dep",nd),("ad",na)]:
            ...
            for ntw in [0,16,32]:
                if nname=="dep" and ntw==32: continue
                t0=time.time()
                if ntw:
                ...
                print(f"  p={p} {nname:3s} tw={ntw:2d}: raw max {np.max(np.abs(Xr-Xe)):.4f} odr max {np.nanmax(np.abs(Xm-Xe)):.4f} | raw rmse {np.sqrt(np.mean((Xr-Xe)**2)):.4f} odr rmse {np.sqrt(np.nanmean((Xm-Xe)**2)):.4f} witness {w:+.4f} ({time.time()-t0:.1f}s)")
