...
# ---------------------------------------------------------------------------
# Split: keys participants legitimately need stay in clear; everything that IS an answer
# (statevectors, Hamiltonian coefficients, energies, the Trotter table, the truncation shift,
# gate counts) is sealed into one opaque blob so the file cannot be read as an answer key.
# See fallfest_grader.seal_bytes: this is obfuscation, not cryptography.
# ---------------------------------------------------------------------------
PUBLIC_PREFIXES = ("tgt_", "isa_circuit_", "window_", "peak_", "dip_")
PUBLIC_KEYS = {
    "X_ref_t8_bd64", "X_ref_t8_bd128", "C_ref", "C_ref_measured", "RMSE_0", "bd64_vs_bd128_max",
    "layout_t8_kingston_O3_seed42",            # the transpiler's default layout (ex 2.3 judge flag)
    "chi_wave_t0_L6", "chi_vacuum_t0_L6", "chi_vacuum_sub_t0_L6",   # L=6 rehearsal truths (ex 3.3)
    "X_trotter_L6_t4", "X_exact_trunc_L6_t4",                        # the notebook computes these openly
    "refs_qiskit_version", "refs_qpy_version", "refs_built",
}

public, sealed = {}, {}
for k, v in store.items():
    (public if (k in PUBLIC_KEYS or k.startswith(PUBLIC_PREFIXES)) else sealed)[k] = v

SEAL_SEED = 20260910
buf = io.BytesIO()
np.savez_compressed(buf, **sealed)
raw = buf.getvalue()
blob = np.bitwise_xor(np.frombuffer(raw, dtype=np.uint8),
                      np.random.default_rng(SEAL_SEED).integers(0, 256, size=len(raw), dtype=np.uint8))
put("refs_qiskit_version", qiskit.__version__)
put("refs_qpy_version", int(qpy_common.QPY_VERSION))
put("refs_built", "2026-09-10")
public["refs_qiskit_version"] = store["refs_qiskit_version"]
public["refs_qpy_version"] = store["refs_qpy_version"]
public["refs_built"] = store["refs_built"]

np.savez_compressed(OUT, _sealed_blob=blob, _sealed_seed=np.int64(SEAL_SEED), **public)
print(f"\nwrote {OUT}: {len(public)} public keys + {len(sealed)} sealed keys "
      f"({os.path.getsize(OUT) / 1e6:.1f} MB) in {time.time() - T0:.1f}s")
print("  public:", ", ".join(sorted(public)))
